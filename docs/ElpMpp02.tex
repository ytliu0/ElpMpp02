...
\subsection{Velocity of the Moon}

If rectangular components of velocity $\dot{X}$, $\dot{Y}$ and $\dot{Z}$ are 
also required, call this subroutine instead: \\ 
{\tt getX2000\_Xdot2000(double T,  Elp\_paras \&paras, Elp\_coefs \&coefs, double (\&Xvec)[6])}.
The first three components of the array {\tt Xvec} stores $X$, $Y$ and $Z$. The last 
three components store $\dot{X}$, $\dot{Y}$ and $\dot{Z}$. Note that the computation 
of velocity doubles the computation time and should be avoided if velocity is not needed.
...
\href{https://ytliu0.github.io/ElpMpp02/ElpMpp02.html}{this JavaScript calculator} (or 
\href{https://ytliu0.github.io/ElpMpp02/ElpMpp02v.html}{this JavaScript calculator} if 
velocity is also needed).
